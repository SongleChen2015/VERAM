...
feature_dir = '/data/featuredata'; % feature extracted from each image
h5path = './modelnet10_144_train.h5'; %the output of this script for shapegram
...
for l = 1:size(labels,2) 
    ...
    while ~feof(listfile)
        ...
        for r = 1:rows
            ...
            for c = 1:cols
                az = (c-1)*30;
                featurepath = sprintf('%s/%s/train/%s_%03d_%03d.txt', feature_dir, class, modelname, el, az);
                disp(featurepath);
